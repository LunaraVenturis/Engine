...
namespace LunaraEngine
{
    ...
    void PipelineBuilder::CreateDescriptorSetLayout(uint32_t location, BufferResourceType type)
    {
        ...
        switch (type)
        {
            case BufferResourceType::Buffer:
                GetBufferDescriptorLayoutBindings(descriptorSetLayoutBindings);
                break;
            case BufferResourceType::Texture:
                GetTextureDescriptorLayoutBindings(descriptorSetLayoutBindings);
                break;
            default:
                break;
        }
        ...
        if (descriptorSetLayoutBindings.empty()) return;

        if (vkCreateDescriptorSetLayout(m_RendererData->device, &layoutInfo, nullptr, &m_DescriptorLayouts[location]) !=
            VK_SUCCESS)
        {
    ...
    void PipelineBuilder::GetTextureDescriptorLayoutBindings(
            std::vector<VkDescriptorSetLayoutBinding>& descriptorSetLayoutBindings)
    {
    ...
    void PipelineBuilder::GetBufferDescriptorLayoutBindings(
            std::vector<VkDescriptorSetLayoutBinding>& descriptorSetLayoutBindings)
    {
    ...
    void PipelineBuilder::CreatePipelineLayout()
    {
        ...
        std::vector<VkDescriptorSetLayout> descriptorLayouts(m_DescriptorLayouts.size(), VK_NULL_HANDLE);
        for (auto& [location, layout]: m_DescriptorLayouts) { descriptorLayouts[location] = layout; }
    ...
    void PipelineBuilder::AddDescriptorSet(uint32_t set, BufferResourceType type) { m_DescriptorSetTypes[set] = type; }

    PipelineData PipelineBuilder::CreatePipeline()
    {
        for (const auto [location, type]: m_DescriptorSetTypes) { CreateDescriptorSetLayout(location, type); }
        ...
        return {pipeline, m_Layout, m_DescriptorLayouts};
    }
